...
from note_categorizer.categorizer.parser import TerminalParser, WebParser, Parser
...
def test_terminal_parser() -> None:
    """Test the parser"""
    parser: Optional[Parser] = TerminalParser.from_json_notation(data_list)
    assert (
        parser is not None
...
def test_web_parser() -> None:
    """Test the parser"""
    parser: Optional[Parser] = WebParser.from_json_notation(data_list)
    assert (
        parser is not None
    ), "Creating parser from json failed due to validation. This shouldn't happen."
    assert parser.get_category_by_name("test") is None
